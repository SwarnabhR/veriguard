#!/usr/bin/env python3
"""VeriGuard - Step 1d: Extract Frames from FACE2FACE Videos"""
import os, cv2
...
print("=" * 70)
print("VERIGUARD - STEP 1d: FACE2FACE")
print("=" * 70)
video_path = RAW_PATH / 'manipulated_sequences/Face2Face/c23/videos'
videos = list(video_path.glob('*.mp4'))
print(f"Processing {len(videos)} videos...")
output_dir = PROCESSED_PATH / 'Face2Face'
output_dir.mkdir(parents=True, exist_ok=True)
total = sum(extract_frames(v, output_dir, 30, 10) for v in tqdm(videos, ncols=70))
print(f"✓ Complete: {total} frames\nNext: python3 scripts/step1_extract_frames_neuraltextures.py")
