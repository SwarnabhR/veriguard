#!/usr/bin/env python3
"""VeriGuard - Step 1e: Extract Frames from NEURALTEXTURES Videos"""
import os, cv2
...
print("=" * 70)
print("VERIGUARD - STEP 1e: NEURALTEXTURES (FINAL)")
print("=" * 70)
video_path = RAW_PATH / 'manipulated_sequences/NeuralTextures/c23/videos'
videos = list(video_path.glob('*.mp4'))
print(f"Processing {len(videos)} videos...")
output_dir = PROCESSED_PATH / 'NeuralTextures'
output_dir.mkdir(parents=True, exist_ok=True)
total = sum(extract_frames(v, output_dir, 30, 10) for v in tqdm(videos, ncols=70))
print(f"✓ Complete: {total} frames\n✓ ALL FRAME EXTRACTION COMPLETE!\nNext: python3 scripts/step2_detect_faces.py")
